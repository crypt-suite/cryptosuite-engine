...
// Dynamic extraction for the rest of the panels
    ...
    //  NEW: TRIGGER THE BYTECODE EXTRACTOR 
const bytecodeContainer = document.getElementById("bytecode-output");
if (bytecodeContainer) {
    const bytecodeData = extractBytecode(fullOutput);
    bytecodeContainer.textContent = bytecodeData;
    
    // Make the text red if it failed, or green if it succeeded
    if (bytecodeData.includes("ERROR")) {
        bytecodeContainer.style.color = "#ff4444";
    } else {
        bytecodeContainer.style.color = "#00ff00"; 
    }
}
//  -------------------------------------- 
...
function extractBytecode(output) {
    const match = output.match(/\[BYTECODE_START\]([\s\S]*?)\[BYTECODE_END\]/);
    if (!match) return "> ERROR: Bytecode generation failed or was not reached.";
    return match[1].trim();
}
...
function extractExec(output) {
    ...
    //  NEW FIX: Delete the raw Bytecode block so it doesn't clutter the execution terminal! 👇
    text = text.replace(/\[BYTECODE_START\][\s\S]*?\[BYTECODE_END\]\n?/g, '').trim();
